...
Definition content_map : Type := list bool -> option X.

Definition singleton (k : list bool) (v : X) : content_map
  := fun k' => if key_eqb k k' then Some v else None.

Definition cbit_union (cbit : nat) (m0 m1 : content_map) : content_map
  := fun k' => if ith_zer cbit k' then m1 k' else m0 k'.

Inductive pmCBT_valid : prefix -> content_map -> CBT X -> Prop :=
| pcv_leaf (k : list bool) (v : X) (H : OneTerminated k)
  : pmCBT_valid (ZeroExt k) (singleton k v) (Leaf X k v)
| pcv_branch (p : list bool) (pl pr : prefix) (ml mr : content_map) (l r : CBT X)
    (Hl : pmCBT_valid pl ml l) (Hr : pmCBT_valid pr mr r)
    (Hpl : is_prefix (p ++ [false]) pl) (Hpr : is_prefix (p ++ [true]) pr)
  : pmCBT_valid (Finite p) (cbit_union (length p) ml mr) (Branch X (length p) l r).

Definition pCBT_valid (p : prefix) (t : CBT X) : Prop :=
  exists m, pmCBT_valid p m t.

Lemma pmCBT_valid_forget_m : forall (p : prefix) (m : content_map) (t : CBT X),
    pmCBT_valid p m t -> pCBT_valid p t.
Proof.
  intros. eexists. eassumption.
Qed.

Definition mCBT_valid (m : content_map) (t : CBT X) : Prop :=
  exists p, pmCBT_valid p m t.

Lemma pmCBT_valid_forget_p : forall (p : prefix) (m : content_map) (t : CBT X),
    pmCBT_valid p m t -> mCBT_valid m t.
Proof.
  intros. eexists. eassumption.
Qed.

Definition CBT_valid (t : @CBT X) : Prop :=
...
Lemma best_key_has_prefix : forall (k p : list bool) (t : @CBT X),
    pCBT_valid (Finite p) t ->
    is_prefix_zer p (find_best_key X k t).
Proof.
  intros k p t. generalize dependent p. induction t.
...
Lemma key_has_prefix : forall (k p : list bool) (m : content_map) (t : CBT X),
...
Lemma best_key_one_terminated : forall (k : list bool) (t : @CBT X),
    CBT_valid t -> OneTerminated (find_best_key X k t).
Proof.
  induction t.
  - intro H. unfold CBT_valid in H. destruct H as [m [p H]]. inversion H. simpl.
    assumption.
  - intro H. simpl. unfold CBT_valid in *. destruct H as [m [p0 H]]. inversion H.
    subst. 
    destruct (ith_zer (length p) k); try eapply IHt1; try eapply IHt2;
...
Proof.
  ...
  - simpl in H. eexists. split. reflexivity. apply Nat.ltb_lt in H. lia.
  - discriminate.
Qed.
...
Proof.
  intros o m H. destruct o.
  - simpl in H. right. eexists. split. reflexivity. apply Nat.ltb_ge. assumption.
  - tauto.
Qed.

Lemma high_critbit_is_prefix : forall (k p : list bool) (t : @CBT X),
    pCBT_valid (Finite p) t ->
    is_beforeb (critical_bit_CBT X k t) (length p) = false ->
    is_prefix_zer p k.
Proof.
  intros k p t H1 H2. destruct H1 as [m H1]. inversion H1. subst.
  apply is_beforeb_false_high_none in H2.
  ...
  remember (find_best_key X k (Branch X (length p) l r)) as k'.
  assert (take_zer (length p) k = take_zer (length p) k').
  { destruct H2.
    - rewrite critical_bit_zer_none_eqv in H. auto.
    - destruct H as [n [H2 H3]]. apply critical_bit_zer_take_eq in H2.
      erewrite <- take_take; [| eassumption]. rewrite H2.
      rewrite take_take; [| assumption]. reflexivity. }
  apply prefix_iff_take_eq. congruence. 
Qed.

Lemma low_critbit_on_prefix : forall (k p : list bool) (t : @CBT X),
    pCBT_valid (Finite p) t ->
    is_beforeb (critical_bit_CBT X k t) (length p) = true ->
    critical_bit_CBT X k t = critical_bit_fin k p.
Proof.
  intros k p t. generalize dependent p. induction t.
  - intros p H. destruct H. inversion H.
  - intros p H1 H2. unfold critical_bit_CBT in *. simpl in *. destruct H1 as [m H1].
    ...
    assert (forall (ps : prefix) (t : @CBT X),
               (forall (p : list bool), pCBT_valid (Finite p) t ->
                                      is_beforeb (critical_bit_zer k (find_best_key X k t)) (length p) = true ->
                                      critical_bit_zer k (find_best_key X k t) = critical_bit_fin k p) ->
               is_beforeb (critical_bit_zer k (find_best_key X k t)) (length p) = true ->
               is_prefix p ps ->
               pCBT_valid ps t ->
               critical_bit_zer k (find_best_key X k t) = critical_bit_fin k p). {
      ...
      - destruct H4 as [m H4]. inversion H4. subst.
        simpl in *. rewrite H5.
        ...
        assumption. lia. rewrite ith_from_take.
        apply critical_bit_zer_ith_neq in H5. assumption. assumption.
      - specialize (H1 p0 H4).
        pose proof (best_key_has_prefix k p0 t H4).
        remember (find_best_key X k t) as k'. clear Heqk' H4. simpl in H3.
        rewrite H5 in *. clear H5. simpl in H1. rewrite Nat.ltb_lt in H1.
        unfold "<" in H1. assert (S n <= length p0). apply is_prefix_fin_length in H3.
        lia. specialize (H1 H0).
        ...
        apply critical_bit_zer_take_eq in H1. assumption. lia.
        rewrite ith_from_take. apply critical_bit_zer_ith_neq in H1.
        assumption. assumption.
    } destruct (ith_zer (length p) k).
    + apply pmCBT_valid_forget_m in Hr. eapply H; eassumption.
    + apply pmCBT_valid_forget_m in Hl. apply (H pl t1 IHt1 H2 Hpl Hl).
Qed.

Lemma pcv_finite_len_num : forall (p : list bool) (plen : nat)
                                  (pl pr : prefix) (l r : CBT X),
    length p = plen -> is_prefix (p ++ [false]) pl -> is_prefix (p ++ [true]) pr ->
    pCBT_valid pl l -> pCBT_valid pr r -> @pCBT_valid (Finite p) (Branch X plen l r).
Proof.
  intros. subst. destruct H2. destruct H3. 
  eexists. econstructor; eassumption.
Qed.

Lemma pmcv_finite_len_num : forall (p : list bool) (plen : nat)
                                   (pl pr : prefix) (ml mr : content_map) (l r : CBT X),
    length p = plen -> is_prefix (p ++ [false]) pl -> is_prefix (p ++ [true]) pr ->
    pmCBT_valid pl ml l -> pmCBT_valid pr mr r ->
    pmCBT_valid (Finite p) (cbit_union plen ml mr) (Branch X plen l r).
Proof.
  intros. rewrite <- H. econstructor; eassumption.
Qed.

Definition cmap_insert (k : list bool) (v : X) (m : content_map)
  : content_map := (fun k' => if key_eqb k k' then Some v else m k').

Lemma cmap_insert_cbit_singleton : forall (k0 k : list bool) (v0 v : X)
                                          (n : nat),
...
Proof.
  intros. unfold cmap_insert. unfold cbit_union.
  apply critical_bit_zer_ith_neq in H. extensionality k''. unfold singleton.
  destruct (key_eqb k k'') eqn:E.
  - apply key_eqb_iff in E. subst k''.
    destruct (ith_zer n k) eqn:E2; apply not_eq_sym in H; rewrite E2;
    rewrite key_eqb_reflx; reflexivity.
  - destruct (ith_zer n k) eqn:E2; apply bneq_eq_neg in H; simpl in H; rewrite E;
    destruct (key_eqb k0 k'') eqn:E3; try (apply key_eqb_iff in E3; subst;
    rewrite <- H; reflexivity); destruct (ith_zer n k''); reflexivity.
Qed.

Lemma cmap_insert_eqk_singleton : forall (k : list bool) (v0 v : X),
...
Lemma cmap_insert_low_cb_branch : forall (k : list bool) (v : X)
                                         (p : list bool) (m : content_map)
                                         (l r : @CBT X) (n : nat),
    pmCBT_valid (Finite p) m (Branch X (length p) l r) ->
    critical_bit_CBT X k (Branch X (length p) l r) = Some n -> n < length p ->
...
Lemma cmap_insert_high_cb_branch : forall (k : list bool) (v : X)
                                          (p : list bool) (ml mr : content_map)
                                          (l r : @CBT X),
    pmCBT_valid (Finite p) (cbit_union (length p) ml mr) (Branch X (length p) l r) ->
    is_beforeb (critical_bit_CBT X k (Branch X (length p) l r)) (length p) = false ->
...
Theorem insert_correct :
  forall (p : prefix) (m : content_map) (t : CBT X) (k : list bool) (v : X)
         (p' : list bool),
    pmCBT_valid p m t -> is_prefix p' p -> is_prefix_zer p' k -> OneTerminated k ->
    exists pi, is_prefix p' pi /\ pmCBT_valid pi (cmap_insert k v m) (insert X k v t).
Proof.
  ...
  - intros p' H1 H2 H3. unfold insert. unfold critical_bit_CBT. simpl.
    ...
    + exists (Finite (take_zer n k)). simpl in *. split.
      ...
      * unfold insert_as_branch. pose proof (critical_bit_zer_ith_neq _ _ _ E).
        ...
        assert (pmCBT_valid (ZeroExt k0) (singleton k0 v0) (Leaf X k0 v0)).
        { constructor. assumption. }
        assert (pmCBT_valid (ZeroExt k) (singleton k v) (Leaf X k v)).
        ...
        -- assumption.
    + eexists. apply critical_bit_zer_none_eq in E; try assumption. split.
      * eassumption.
        * subst k. rewrite cmap_insert_eqk_singleton. constructor. assumption.
  - intros p' H1 H2 H3. unfold insert. simpl. 
    destruct ((is_beforeb (critical_bit_CBT X k (Branch X (length p) l r))) (length p)) eqn:E.
    + pose proof (low_critbit_on_prefix k p (Branch X (length p) l r)).
      specialize (H4 (pmCBT_valid_forget_m _ _ _ (pcv_branch p pl pr ml mr l r H H0 Hpl Hpr)) E). rewrite H4 in *. apply is_beforeb_true_exists in E. destruct E as [n [H5 H6]].
      ...
      * simpl. simpl in H1. assert (length p' <= n).
        ...
        rewrite ith_from_take in H7. contradiction. assumption.
      ...
      * unfold insert_as_branch.
        ...
        { apply critical_bit_fin_zer in H5. apply critical_bit_zer_take_eq. assumption. }
...
Theorem lookup_correct : forall (p : prefix) (m : content_map) (t : CBT X)
                                (k : list bool),
    OneTerminated k -> pmCBT_valid p m t -> lookup X k t = m k.
Proof.
  intros. induction H0.
